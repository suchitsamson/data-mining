...
first = data.first()
if(case == 1):
    csvRDD1 = data.filter(lambda line : line != first).map(lambda line: (line.split(',')[0], line.split(',')[1]))
if(case == 2):
    csvRDD1 = data.filter(lambda line : line != first).map(lambda line: (line.split(',')[1], line.split(',')[0]))

csvRDD=csvRDD1.map(lambda x: (x[0],[x[1]])).reduceByKey(lambda x,y: x+y)
